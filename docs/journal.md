...
> Useful for junior devs to understand _why_ things were done a certain way.
...
- Before writing a single line of code, you need to understand the _domain_. SpendBook isn't just a CRUD app — it has a specific **loan balance calculation** that depends on payment mode ownership and "paid towards" fields.
- Skipping this step leads to wrong abstractions, missed edge cases, and expensive rewrites later.

**Interview insight:** When asked "How do you approach a new project?", the answer is: _read the requirements, understand the domain model, then plan the architecture before coding._
...
- **`server/queries/`** holds data-fetching functions. These are called from Server Components to load data. They're _not_ Server Actions — they're regular async functions.
- This separation makes it obvious: _actions mutate, queries read_.
...
- `journal.md` (this file) captures the _thinking_ behind decisions. Code tells you _what_ was done; the journal tells you _why_.
...
- With good instructions, AI becomes a 10x productivity tool — it generates code that _fits_ your architecture.
...
**Interview insight:** AI-assisted development is a real skill. Knowing how to _guide_ an AI (prompt engineering, context setting) is increasingly valuable.
...
- They communicate intent at a glance — `feat` vs `fix` vs `refactor` tells you _what kind_ of change it is.
...
**Interview insight:** This is a classic interview topic: "How do you handle money in software?" The answer: _never use floats. Use Decimal types or integer cents._
...
- We _can_ layer it on later, but the application should enforce tenant isolation regardless. RLS is a safety net, not the primary mechanism.
...
1. **Plan before you code.** We spent an entire session _just_ setting up structure and documentation. No app code was written. This investment pays off massively in consistency and speed later.
...
4. **Document decisions, not just code.** Code changes; the _reasoning_ behind decisions is what junior devs (and future you) actually need to understand the system.
