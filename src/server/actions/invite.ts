...
export async function createInvite(role: Role): Promise<ActionResult<string>> {
...
export async function acceptInvite(token: string, personName: string): Promise<ActionResult<void>> {
  const session = await getAppSession();
  if (!session?.user) return { success: false, error: "Unauthorized" };
