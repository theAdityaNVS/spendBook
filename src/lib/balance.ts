...
import type { Transaction, PaymentMode } from "@/generated/prisma";
 ...
 */
export function computeLoanDelta(tx: TransactionForBalance, personId: string): Decimal {
...
export async function recalculateAllBalancesForDate(familyId: string, date: Date): Promise<void> {
  const persons = await db.person.findMany({
    where: { familyId, isArchived: false },
  });

  await Promise.all(persons.map((p) => recalculateBalancesForDate(familyId, p.id, date)));
}
